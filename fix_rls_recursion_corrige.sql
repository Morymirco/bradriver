-- =====================================================
-- CORRECTION RLS BASÉE SUR LE NOUVEAU SCHÉMA
-- =====================================================
...
-- 2. DÉSACTIVER RLS SUR TOUTES LES TABLES POUR ÉVITER LA RÉCURSION
-- ================================================================
...
-- Politique simple pour voir toutes les commandes
CREATE POLICY "drivers_can_view_orders" ON orders
FOR SELECT TO authenticated
USING (true);

-- Politique pour mettre à jour les commandes
...
SELECT 
  ...
  rowsecurity
FROM pg_tables 
WHERE tablename IN ('orders', 'businesses', 'user_profiles', 'order_items', 'driver_profiles', 'notifications')
...
-- Test 7: Voir les notifications (sans RLS)
SELECT 
  'Test 7 - Notifications' as test,
...
-- 9. VÉRIFIER LES RELATIONS
-- ==========================
-- Voir les clés étrangères de la table orders
SELECT 
  tc.constraint_name,
  tc.table_name,
  kcu.column_name,
...
-- =====================
SELECT 
  '✅ RLS CORRIGÉ SELON LE NOUVEAU SCHÉMA' as message,
  'RLS désactivé sur toutes les tables sauf orders' as details;
